...
def find_joint_angles(thetas_init, desired_end_effector_pose, dh_params, smart_seed=True, tolerance=1e-06, seeding_max_iters=100):
    solved = False
    ...
    if not solved:
        print("------------------------------------------")
        print(f"error greater than {tolerance} at {error}")
        if smart_seed:
            print(
                f"attempting seeding optimization to fix error")
            num_joints = len(dh_params[0])
            for i in range(seeding_max_iters):
                ...
                if new_error <= tolerance:
                    solved = True
                    print(
                        f"seeding optimization **successful**")
                    print(f"Error less than tolerance at {error}")
                    print(f"number of seeding iters it took is {i + 1}")
                    break
        print("------------------------------------------")
        print()
...
def draw_links(plotter, joint_positions):
    """Given an array of joints, draw the links between each joint"""
    poly = pv.PolyData()
...
def draw_robot_arm(thetas, p, dh_params):
...
def animate_inverse_kinematics_sphere(dh_params):
    ...
    def callback(idx, pose_param):
        ...
        thetas, solved = find_joint_angles(
            thetas_init, desired_end_effector_pose_in_quat, dh_params, smart_seed=False)
        if solved:
            thetas_init[:] = thetas
...
def animate_inverse_kinematics_sliders(dh_params, position_bound=700):
    ...
    def callback(idx, pose_param):
        ...
        desired_end_effector_pose_quat = convert_pose_to_quat(
            desired_end_effector_pose)
        thetas, error = find_joint_angles(
            thetas_init, desired_end_effector_pose_quat, dh_params, smart_seed=False)
        if error < 0.1:
            thetas_init[:] = thetas
        draw_robot_arm(thetas, p, dh_params)
...
def wrap_joint_angles(joint_angles, radians=True):
    if radians:
        period = np.pi
    else:
        period = 180
